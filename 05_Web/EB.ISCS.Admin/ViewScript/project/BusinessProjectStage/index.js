...
(function () {
    ...
    app.controller("ListController", function ($scope, commonService) {
        list.InitializePage($scope, commonService);
    });
    ...
    var list = {
        ...
        InitGrid: function () {
            var _this = this;
            gridView.gvLoadGrid({
                gvHeadXml: "Project\\项目阶段管理",
                gvUrl: "/BusinessProjectStage/GetList",
        ...
        AddEventListener: function () {
            ...
            $("#btn_del").click(function () {
                ...
                gridView.tgvDeleteRowsUrl(
                    "/BusinessProjectStage/DelData/" + row.Id,// 删除url
                    function () {
            ...
            $('#btn_task').click(function () {
                if (gridView.gvIsSelectRow()) {
                    var row = gridView.gvGetSelectRow();
                    _this.showTaskEdit(row.Id, "编辑项目阶段任务信息");
                }
            });
        ...
        showEdit: function (id, title) {
            var _this = this;
            var url = "/BusinessProjectStage/Edit?id=" + id + "&_t=" + Math.random();
            id = id ? id : 0;
            layer.open({
                type: 2,
                title: title,
        ...
        showTaskEdit: function (id, title) {
            var _this = this;
            var url = "/BusinessProjectStage/EditTask?id=" + id + "&_t=" + Math.random();
            id = id ? id : 0;
            layer.open({
                type: 2,
                title: title,
