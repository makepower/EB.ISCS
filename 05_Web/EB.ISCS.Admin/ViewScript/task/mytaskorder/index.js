...
(function () {
    ...
    var app = angular.module("MyTaskOrderApp", ["CommonApp"]);
    ...
    var employeeList = {
        ...
        InitGrid: function () {
            var _this = this;
            gridView.gvLoadGrid({
                gvHeadXml: "Task\\工单任务信息",
                gvUrl: "/MyTaskOrder/TaskOrderList",
        ...
        AddEventListener: function () {
            ...
            $("#btn_del").click(function () {
                ...
                if (row.Id) {
                    $.post("/MyTaskOrder/FormDelete/" + row.Id, function (result) {
            ...
            $("#btn_create").click(function () {
                ...
                if (gridView.gvIsSelectRow()) {
                    ...
                    var layerFr = layer.open({
                        ...
                        area: ["1100px", he + "px"],
                        content: ["/MyTaskOrder/CreateForm/" + row.Id, "no"]
                    });
                    layer.full(layerFr);
                }
            });
            ...
            $("#rulesTree").tree({
                url: "/MyTaskOrder/GetRulesTree/1300",
                method: "get",
                animate: true,
                lines: true,
        ...
        showEdit: function (id, title) {
            var _this = this;
            var url = "/MyTaskOrder/Edit?id=" + id + "&_t=" + Math.random();
            id = id ? id : 0;
            if (id > 0) {
                // edit
                var url = "/MyTaskOrder/NewTask?id=" + id + "&state=1" + "&_t=" + Math.random();
                layer.open({
                    type: 2,
                    title: "执行任务",
                    area: ["1000px", "720px"],
                ...
                layer.open({
                    ...
                    content: [url, "no"],
                        ...
                        var c = $('body').data("task_create_flow_id");
                        if (c) {
                            var url = "/MyTaskOrder/NewTask?id=" + c + "&state=0" + "&_t=" + Math.random();
                            layer.open({
                                type: 2,
                                title: "执行任务",
                                area: ["1000px", "720px"],
    ...
    //控制器
    app.controller("MyTaskOrderController", function ($scope, commonService) {
