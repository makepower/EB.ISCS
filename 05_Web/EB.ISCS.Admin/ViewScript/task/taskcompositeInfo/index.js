...
(function () {
    ...
    var app = angular.module('TaskCompositeListApp', ['CommonApp']);

    //复合任务管理
    var taskCompositeList = {
        gridView: null,
        where: null,
        _scope: null,
        _commonService: null,
        ...
        InitGrid: function () {
            var _this = this;
            gridView.gvLoadGrid({
                gvHeadXml: "Task\\单线任务管理",// 复用表头
                gvUrl: "/TaskCompositeInfo/PageList",
        ...
        AddEventListener: function () {
            ...
            $("#btn_createflow").click(function () {
                ...
                if (gridView.gvIsSelectRow()) {
                    var row = gridView.gvGetSelectRow();
                    comControl.openTabpage("TaskCompositeInfo/CreateFlow/" + row.Id, "编辑【" + row.Name + "】流程");
                }
            });
        ...
        showEdit: function (id, title) {
            var _this = this;
            var url = "/TaskCompositeInfo/Edit?id=" + id + "&_t=" + Math.random();
            id = id ? id : 0;
            layer.open({
                type: 2,
                title: title,
    ...
    //控制器
    app.controller("TaskCompositeListController", function ($scope, commonService) {
        taskCompositeList.InitializePage($scope, commonService);
    });
})();
