...
(function () {
    var app = angular.module('CreateFlowApp', ['CommonApp']);

    //控制器
    app.controller('CreateFlowController', function ($scope, service, commonService) {
    ...
    app.factory('service', ['$http', function ($http) {
        return {
            //获取流程
            getFlow: function (id) {
                return $http.post('/FlowTemplate/GetFlowModelById/' + id);
            },
            //保存流程数据
            saveFlow: function (postData) {
                return $http.post("/FlowTemplate/SaveFlowData", postData);
            }
        }
    }]);
        ...
        AddEventListener: function () {
            ...
            $("#btn_save").click(function () {
                if (_goo.comFlowHasNode() > 0) {
                    ...
                    var datas = {
                        Info: { Id: _scope.id, Content: JSON.stringify(d.data) },
                        List: d.list
                    };
        ...
        InitData: function (id) {
            ...
            _service.getFlow(id).success(function (result) {
                if (result.Data.Info.Content) {
                    _goo.onLoadData(result.Data.Info.Content, result.Data.List);
                }
            });
        },
        property: {
